...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // dev3 — variant switcher (A/B/C) via ?v= URL param
    const params = new URLSearchParams(location.search);
    const variant = params.get('v') || 'a';
    document.body.classList.add('scrubber-variant-' + variant);
    document.querySelectorAll('.dev3-switcher__btn').forEach(btn => {
        if (btn.getAttribute('data-v') === variant) btn.classList.add('is-active');
    });

    // Section label mapping for scrubber
    const sectionLabels = {
        'main': 'Главная',
    ...
    const labelNodes = document.querySelectorAll('[data-scrubber-label]');
    const barNode = document.querySelector('[data-scrubber-bar]');
    if (scrubber && scrubberTrack && scrubberThumb) {
        const updateScrubberFromScroll = () => {
            ...
            labelNodes.forEach(n => { if (n.textContent !== currentLabel) n.textContent = currentLabel; });
            if (barNode) barNode.style.width = (pct * 100) + '%';
        };
