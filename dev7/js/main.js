...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /* ===================================
       Full-Page Snake — SVG path through all sections
       =================================== */
    (function initFullSnake() {
        // Create full-page SVG overlay
        const svgEl = document.createElement('div');
        svgEl.id = 'fullSnakeWrap';
        svgEl.style.cssText = 'position:absolute;top:0;left:0;width:100%;pointer-events:none;z-index:5;overflow:visible;';
        ...
        function getWaypoints() {
            ...
            // Process snake circles
            const snakeSteps = document.querySelectorAll('.snake-step__circle');
            snakeSteps.forEach(circle => {
                const r = circle.getBoundingClientRect();
                points.push({ x: r.left + r.width / 2, y: r.top + scrollY + r.height / 2 });
            });

            // Transition: exit process → enter advantages header
            const advHeader = document.querySelector('#advantages .section__title');
            if (advHeader) {
                const r = advHeader.getBoundingClientRect();
                points.push({ x: pageW * 0.5, y: r.top + scrollY - 30 });
                // Underline the heading
                points.push({ x: r.left + 20, y: r.top + scrollY + r.height + 6 });
                points.push({ x: r.right - 20, y: r.top + scrollY + r.height + 6 });
            }

            // Advantages first and last card
            const advCards = document.querySelectorAll('.advantage-card');
            if (advCards.length > 0) {
                const first = advCards[0].getBoundingClientRect();
                points.push({ x: first.left + first.width / 2, y: first.top + scrollY + 30 });
                const last = advCards[advCards.length - 1].getBoundingClientRect();
                points.push({ x: last.left + last.width / 2, y: last.top + scrollY + last.height - 30 });
            }

            // Portfolio header
            const portHeader = document.querySelector('#portfolio .section__title');
            if (portHeader) {
                const r = portHeader.getBoundingClientRect();
                points.push({ x: pageW * 0.5, y: r.top + scrollY - 20 });
                points.push({ x: r.left + 20, y: r.top + scrollY + r.height + 6 });
                points.push({ x: r.right - 20, y: r.top + scrollY + r.height + 6 });
            }

            // Portfolio first card center
            const portCards = document.querySelectorAll('.portfolio-card');
            if (portCards.length > 0) {
                const first = portCards[0].getBoundingClientRect();
                points.push({ x: first.left + first.width / 2, y: first.top + scrollY + first.height / 2 });
            }
        ...
        function rebuild() {
            ...
            svgEl.innerHTML = `<svg xmlns="${svgNS}" width="${pageW}" height="${pageH}" viewBox="0 0 ${pageW} ${pageH}" style="position:absolute;top:0;left:0;">
                <path d="${d}" class="snake-path" fill="none" stroke="#ae8c56" stroke-width="2" stroke-linecap="round" opacity="0.6"/>
            </svg>`;

            pathEl = svgEl.querySelector('.snake-path');
        ...
        function animateOnScroll() {
            ...
            // Activate snake steps
            const steps = document.querySelectorAll('.snake-step');
            steps.forEach((step, i) => {
                const threshold = 0.05 + (i * 0.06);
                step.classList.toggle('is-active', progress >= threshold);
            });
        }

        // Init with delay for layout
        setTimeout(rebuild, 300);
        window.addEventListener('resize', () => { setTimeout(rebuild, 100); });
        window.addEventListener('scroll', animateOnScroll, { passive: true });
    })();

    /* Legacy snake timeline compat — activate steps */
    ...
    if (snakeTimeline && snakeSvg) {
        // Hide the local SVG (full-page snake replaces it)
        snakeSvg.style.display = 'none';
        const steps = snakeTimeline.querySelectorAll('.snake-step');
        // Steps activation handled by full-page snake above
